...
Query(Commit)
.distinct('')
# Q 4 : # @commits.select(&:has_signature?).map
Query(Commit)
.select('has_signature?')
# Q 5 : # @commits.select(&:has_signature?).map
Query(Commit)
.select('has_signature?')
# Q 6 : # @discussions.select(&:resolvable?)
Query(Discussion)
.select('resolvable?')
...
Query(Deployment)
.where("project_id = ?")
# Q 18 : # @project.find_or_initialize_service("mattermost_slash_commands")
Query(Project)
...
.where("project_id = ?")
.where("id = ?")
# Q 48 : # @project.touch(:last_activity_at, :last_repository_updated_at)
Query(Project)

# Q 49 : # @project.uploads.where("path != ?", avatar_path)
Query(Project)
...
Query(CommitStatus)
.where("id = ?")
# Q 67 : # CommonMetrics::PrometheusMetric.common.find_by(identifier: id)
Query(CommonMetrics::PrometheusMetric)
.select('common')
.where("identifier = ?")
# Q 68 : # ConversationalDevelopmentIndex::Metric.order(:created_at).last.present
Query(ConversationalDevelopmentIndex::Metric)
.order('created_at')
.return_limit('1')
...
.where("created_at = ?")
.where("project_id = ?")
# Q 79 : # Event.from_union([project_events, group_events]).recent
Query(Event)

# Q 80 : # Event.from_union([repo_events, issue_events, mr_events, note_events]).map(&:attributes)
Query(Event)
...
Query(Event)
.order('')
.select('date(created_at + ? ) AS datecount(id) as total_amount')
...
.distinct('')
.order('')
# Q 83 : # Event.unscoped.recent
Query(Event)

# Q 84 : # GPGME::Key.find(:public, fingerprints)
Query(GPGME::Key)
.where("id = ?")
...
Query(Group)
.where("namespace_id = ?")
# Q 112 : # Group.select([namespaces[:id], greatest(members[:access_level], cte.table[:access_level], "access_level")]).joins(join_cte(cte)).joins(join_members_on_namespaces).except(:order)
Query(Group)
...
.order('access_level')
.return_limit('1')
# Q 121 : # Identity.with_extern_uid(auth_hash.provider, auth_hash.uid).take
Query(Identity)
.return_limit('1')
...
Query(Namespace)
.joins('projects')
# Q 188 : # Namespace.unscoped.select([links[:project_id], least(cte_alias[:access_level], links[:group_access], "access_level")]).from(cte_alias).joins("INNER JOIN project_group_links ON project_group_links.group_id = namespaces.id").joins("INNER JOIN projects ON projects.id = project_group_links.project_id").joins("INNER JOIN namespaces p_ns ON p_ns.id = projects.namespace_id").where("p_ns.share_with_group_lock IS FALSE")
Query(Namespace)
...
.where("source_type = ?")
.where("source_id = ?")
# Q 202 : # OauthAccessToken.distinct_resource_owner_counts(@applications)
...
# Q 209 : # Project.eager_load(:route, namespace: [:route]).where_full_path_in(projects).index_by(&:full_path).transform_keys(&:downcase)
Query(Project)

# Q 210 : # Project.eager_load(:route, namespace: [:route]).where_full_path_in(unique_project_paths).index_by(&:full_path)
Query(Project)
...
Query(Project)
.where("full_path = ?")
# Q 232 : # Project.find_by_full_path(match[:project_path])
Query(Project)
.where("full_path = ?")
# Q 233 : # Project.find_by_full_path(params[:project])
Query(Project)
.where("full_path = ?")
# Q 234 : # Project.find_by_full_path(project_full_path)
Query(Project)
.where("full_path = ?")
# Q 235 : # Project.find_by_full_path(project_path)
Query(Project)
.where("full_path = ?")
# Q 236 : # Project.find_by_full_path(project_path, follow_redirects: true)
Query(Project)
.where("full_path = ?")
# Q 237 : # Project.find_by_full_path(ref)
Query(Project)
.where("full_path = ?")
...
Query(Project)
.where("id = ?")
# Q 243 : # Project.inside_path(group.full_path)
Query(Project)
...
Query(Project)
.select('id')
# Q 261 : # Project.with_route.where(id: deploy_keys_projects.with_write_access.select(:project_id))
Query(Project)
.where("id = ?")
# Q 262 : # Project.with_storage_feature(:repository).in_batches(of: batch_size, start: range_from, finish: range_to)
Query(Project)

# Q 263 : # Project.with_unmigrated_storage.in_batches(of: batch_size, start: range_from, finish: range_to)
Query(Project)
...
Query(Image)
.select('valid?')
# Q 364 : # builds.skipped.after_stage(stage_idx).find_each(&:process)
Query(Build)
...
Query(ZoomMeeting)
.return_limit('1')
# Q 371 : # chat_names.find(params[:id])
Query(ChatName)
.where("id = ?")
...
Query(Project)
.select('id')
# Q 426 : # from_union(relations).select(["project_id", "MAX(access_level) AS access_level"]).group(:project_id)
Query(ProjectAuthorization)
.select('project_id
...
.where("source = ?")
.where("id = ?")
# Q 442 : # groups.except(:order)
Query(Group)

# Q 443 : # groups.pluck(:id)
Query(Group)
.select('id')
# Q 444 : # groups.reorder(order_options)
Query(Group)
.order('')
...
Query(GpgKey)
.includes('subkeys')
# Q 454 : # issue.update_attribute(:label_ids, issue_labels.pluck(:id))
Query(Issue)

# Q 455 : # issue.update_attribute(:label_ids, issue_labels.pluck(:id))
Query(Issue)

# Q 456 : # issues.joins(:timelogs).sum(:time_spent)
Query(Issue)
...
Query(Issue)
.where("id = ?")
# Q 460 : # iwhere(extern_uid: normalize_uid(provider, extern_uid)).with_provider(provider)
Query(Identity)
...
# Q 466 : # joins("INNER JOIN trending_projects ON projects.id = trending_projects.project_id").reorder("trending_projects.id ASC")
Query(Project)
.order('id')
# Q 467 : # joins("LEFT JOIN project_authorizations ON users.id = project_authorizations.user_id").where(project_authorizations: { user_id: nil })
Query(User)
.where("user_id = ?")
...
Query(Namespace)
.group('id')
.select('namespaces.*COALESCE(SUM(ps.storage_size), 0) AS storage_sizeCOALESCE(SUM(ps.repository_size), 0) AS repository_sizeCOALESCE(SUM(ps.wiki_size), 0) AS wiki_sizeCOALESCE(SUM(ps.lfs_objects_size), 0) AS lfs_objects_sizeCOALESCE(SUM(ps.build_artifacts_size), 0) AS build_artifacts_sizeCOALESCE(SUM(ps.packages_size), 0) AS packages_size')
# Q 470 : # joins("LEFT OUTER JOIN u2f_registrations AS u2f ON u2f.user_id = users.id").where("u2f.id IS NULL AND users.otp_required_for_login = ?", false)
Query(User)
...
.where("sha = ?")
.order('')
# Q 483 : # joins(:metadata).where("ci_builds_metadata.id NOT IN (?)", Ci::BuildMetadata.scoped_build.with_interruptible.select(:id))
Query(Build)
...
# Q 506 : # key.group.order.num_bits
Query(Key)
.group('')
.order('')
# Q 507 : # label.to_reference(label.group, target_project: resource_parent, format: :id)
Query(Label)
...
.order('id, title')
.select('title')
# Q 513 : # latest.ordered.includes(project: :namespace)
Query(CommitStatus)
